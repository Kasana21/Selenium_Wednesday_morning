package week10_StaticVC_Instance_Encapsulation.PizzaTaks;
...
public class Pizza {
    ...
    public void setNumberOfPepperoniTopping(int numberOfPepperoniTopping) {
        if(numberOfPepperoniTopping<0){
            System.err.println("Number of topping can n ot be negative");
            System.exit(1);
        }
        if(size.equalsIgnoreCase("small")){
            if(numberOfPepperoniTopping > 4){
                System.err.println("Number of pepperoni topping can not be more than three for small sized pizzas");
                System.exit(1);
            }
        } else if (size.equalsIgnoreCase("medium")) {
            if(numberOfPepperoniTopping > 5){
                System.err.println("Number of pepperoni topping can not be more than four for medium sized pizzas");
                System.exit(1);
            }
        }else{
            if(numberOfPepperoniTopping > 5){
                System.err.println("Number of pepperoni topping can not be more than five for large sized pizzas");
                System.exit(1);
            }
        }
...
/*
 Create class named Pizza:
                ...
                Encapsulate all the fields
                        Conditions:
                            size of the pizza can only be small, medium, large. caseinsensitive
